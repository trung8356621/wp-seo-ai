    ...
    public function check(bool $force_refresh = false): array
    {
        ...
        if (($remote['ok'] ?? false) !== true) {
            return [
                ...
                'latest_version' => null,
                'release_version' => $remote['release_version'] ?? null,
                'update_available' => false,
                'release_url' => $remote['release_url'] ?? GitHub_Release_Client::RELEASES_HTML,
                'package_url' => null,
                'expected_asset' => $remote['expected_asset'] ?? null,
                'found_assets' => is_array($remote['found_assets'] ?? null) ? $remote['found_assets'] : [],
                'changelog' => '',
        ...
        $payload = [
            ...
            'expected_asset' => (string) ($remote['expected_asset'] ?? GitHub_Release_Client::expected_asset_name($latest)),
            'found_assets' => is_array($remote['found_assets'] ?? null) ? $remote['found_assets'] : [],
    ...
    public function install(string $operationId): array
    {
        ...
        if ($packageUrl === '' || $latest === '') {
            return [
                ...
                'release_version' => $latest !== '' ? $latest : ($check['release_version'] ?? null),
                'expected_asset' => $check['expected_asset'] ?? ($latest !== '' ? GitHub_Release_Client::expected_asset_name($latest) : null),
                'found_assets' => is_array($check['found_assets'] ?? null) ? $check['found_assets'] : [],
                'updated' => false,
            ];
        }
